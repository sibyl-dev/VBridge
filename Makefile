...
lint: ## check style with flake8 and isort
	flake8 vbridge tests
	isort -c --recursive vbridge tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find vbridge tests -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive vbridge tests
	isort --apply --atomic --recursive vbridge tests

.PHONY: coverage
coverage: ## check code coverage quickly with the default Python
	coverage run --source vbridge -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --separate --no-toc -o docs/api/ vbridge
	$(MAKE) -C docs html
