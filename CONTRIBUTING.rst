...
VBridge could always use more documentation, whether as part of the
official VBridge docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `VBridge` for local development.

1. Fork the `VBridge` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/vbridge.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv vbridge
    $ cd vbridge/
...
2. The tests that cover a module called ``vbridge/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/vbridge/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_vbridge
...
2. Update the version in ``setup.cfg``, ``vbridge/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``vbridge/__init__.py``
...
or in command line::
    ...
    pip install 'vbridge>=X.Y.Z.dev'


.. _GitHub issues page: https://github.com/DAI-Lab/vbridge/issues
.. _Build Status page: https://github.com/DAI-Lab/vbridge/actions
